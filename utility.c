 ...
 * memory for a character string
 * with the specified size. It assigns the allocated memory to
 ...
 *
 * Description: This function dynamically allocates
 * memory for an array of strings
 * with the specified size. It assigns the allocated
 * memory to the reference @array.
 * If the allocation fails, it displays the error message and returns NULL.
 *
 ...
 *
 * Description: This function copies the strings from
 * the source array to the destination array.
 * It dynamically allocates memory for each string in
 * the destination array and performs the copy.
 * If the memory allocation fails, it displays an error
 * message and frees the previously allocated memory.
 *
...
int copy_array(char **destination, char **source)
{
	...
	for (new_env_index = 0; source[new_env_index]; new_env_index++)
	{
		...
		if (destination[new_env_index] == NULL)
		{
			...
			for (free_new_env_index = 0;
					free_new_env_index < new_env_index;
					free_new_env_index++)
 ...
 * specified environment variable @name exists.
 * It searches through the __environ array for a matching name.
