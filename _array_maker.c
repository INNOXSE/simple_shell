...
/**
 * number_of_tokens - Returns the number of tokens in a string
 * @input: Input string
 * @delimiter: Delimiter string
 *
 * Return: Number of tokens in the string, or -1 on failure
 */
...
/**
 * array_maker - Creates a NULL-terminated array of tokens from a string
 * @input: Input string
 * @delimiter: Delimiter string
 *
 * Return: The token array on success, NULL on failure
 */
char **array_maker(char *input, char *delimiter)
{
	...
	{
	array_of_tokens[token_index] = (char *)
		malloc(sizeof(char) * strlen(token) + 1);
	if (array_of_tokens[token_index] == NULL)
	{
	for (token_free_index = 0; token_free_index < token_index;
			token_free_index++)
	{
